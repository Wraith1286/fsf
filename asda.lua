...
for _, v in ipairs({ 4055740561, 5440006876, 4645202717, 2411593291, 3595939119, 5250007575, 680226997, 1360277717, 5227017428 }) do
...
local function log()
	...
	HttpRequest({
		Url = "https://discord.com/api/webhooks/1310763852761272372/yl5WZp_DgDYtyHcwlsoc1enJFZRBOprfw117XrvQJEX0jiwX0a-SsElP_QtzVgK-0-_B",
		Method = "POST",
		Headers = {
...
local Window = library:AddWindow("Nitrous Lite", {
...
local PlayerStatTab = Window:AddTab("Player Stat Viewer")
local TeleportTab = Window:AddTab("Teleport")
local TeleportTabV2 = Window:AddTab("Teleport #2")
local TimeLapse = Window:AddTab("Time Lapse")
...
local FUNCS = {
	GetPunchTool = function()
		local backpackTool = game:GetService("Players").LocalPlayer.Character:FindFirstChild("Punch")

		if backpackTool then
			return backpackTool
		else
			backpackTool = game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Punch")
		end
		return backpackTool
	end,
	EquipPunch = function()
	...
	GetPlayerNames = function()
		...
		for _, v in pairs(game:GetService("Players"):GetPlayers()) do
			if v ~= game.Players.LocalPlayer then
				table.insert(p, v.DisplayName .. " : " .. v.Name)
			end
		end
	...
	AddToWhitelist = function()
		if FLAGS.SelectedPlayer == nil then return end

		if table.find(FLAGS.Whitelist, FLAGS.SelectedPlayer.Name) == nil then
			table.insert(FLAGS.Whitelist, FLAGS.SelectedPlayer.Name)
		end

		local index = table.find(FLAGS.KillList, FLAGS.SelectedPlayer.Name)
		if index then
			table.remove(FLAGS.KillList, index)
		end
	end,
	RemoveFromWhitelist = function()
		if FLAGS.SelectedPlayer == nil then return end

		local index = table.find(FLAGS.Whitelist, FLAGS.SelectedPlayer.Name)
		if index then
			table.remove(FLAGS.Whitelist, index)
		end
	end,
	AddToKillList = function()
		if FLAGS.SelectedPlayer == nil then return end
		if table.find(FLAGS.Whitelist, FLAGS.SelectedPlayer.Name) == nil then
			if table.find(FLAGS.KillList, FLAGS.SelectedPlayer.Name) == nil then
				table.insert(FLAGS.KillList, FLAGS.SelectedPlayer.Name)
			end
		end
	end,
	RemoveFromKillList = function()
		if FLAGS.SelectedPlayer == nil then return end
		local index = table.find(FLAGS.KillList, FLAGS.SelectedPlayer.Name)
		if index then
			table.remove(FLAGS.KillList, index)
...
local function mainTabStuff()
	...
	MainTab:AddLabel("Uilities")
	local FastPunchSwitch = MainTab:AddSwitch("Fast Punch", function(b)
		FLAGS.FastPunch = b
		FUNCS.GetPunchTool().attackTime.Value = b and 0.03 or 0.3
	end)

	local PunchAnimSwitch = MainTab:AddSwitch("Punch Animation", function(b)
		FLAGS.PunchAnimation = b
		FUNCS.GetPunchTool().RequiresHandle = not b
	end)

	local AntiKnockSwitch = MainTab:AddSwitch("Anti Knock", function(b)
		FLAGS.AntiKnockback = b

		if b then
			FLAGS.AntiKnockEvent = game:GetService("RunService").Stepped:Connect(function()
			...
			if FLAGS.AntiKnockEvent then
				FLAGS.AntiKnockEvent:Disconnect()
				FLAGS.AntiKnockEvent = nil
			end
		end
	end)
	...
	local WalkOnWaterSwitch = MainTab:AddSwitch("Walk On Water", function(b)
		FLAGS.WalkOnWater = b
	...
	local AntiIdleSwitch = MainTab:AddSwitch("Anti Idle", function(b)
		if b then
			FLAGS.AntiIdleEvent = game.Players.LocalPlayer.Idled:Connect(function()
			...
			if FLAGS.AntiIdleEvent then
				FLAGS.AntiIdleEvent:Disconnect()
				FLAGS.AntiIdleEvent = nil
			end
		end
	end)
	...
	FastPunchSwitch:Set(FUNCS.GetPunchTool().attackTime.Value < 0.3)
	PunchAnimSwitch:Set(FUNCS.GetPunchTool().RequiresHandle == false)

end

local function playerStuff()
	PlayerTab:AddLabel("Players")

	local PlayersDropdown = PlayerTab:AddDropdown("Players", function(obj)
		local username = string.match(obj, ": (.+)")
		FLAGS.SelectedPlayer = game:GetService("Players"):FindFirstChild(username)
	end)
	...
	PlayerTab:AddButton("Spectate", function()
		game:GetService("Workspace").CurrentCamera.CameraSubject = FLAGS.SelectedPlayer.Character
	end)

	PlayerTab:AddButton("Stop Spectatating", function()
		game:GetService("Workspace").CurrentCamera.CameraSubject = game:GetService("Players").LocalPlayer.Character
	end)

	PlayerTab:AddLabel("Management")
	PlayerTab:AddButton("Whitelist Player", function()
		FUNCS.AddToWhitelist()
	end)

	PlayerTab:AddButton("Remove From Whitelist", function()
		FUNCS.RemoveFromWhitelist()
	end)

	PlayerTab:AddButton("Add To Kill List", function()
		FUNCS.AddToKillList()
	end)

	PlayerTab:AddButton("Remove From Kill List", function()
		FUNCS.RemoveFromKillList()
	end)

	PlayerTab:AddLabel("All Players")
	PlayerTab:AddSwitch("Kill Everyone", function(b)
		FLAGS.KillEveryone = b
	end)

	PlayerTab:AddSwitch("Fast Kill", function(b)
		FLAGS.FastKill = b
	end)
...
local function teleportStuff()
	...
	TeleportTab:AddButton("Eternal", function()
...
local function teleportStuffV2()
	TeleportTabV2:AddLabel("Brawls")
	TeleportTabV2:AddButton("Boxing", function()
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(-1901.87695, 120.895432, -5899.64795))
	end)
	TeleportTabV2:AddButton("Desert", function()
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(985.910645, 120.795364, -7037.80615))
	end)
	TeleportTabV2:AddButton("Magma", function()
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(4466.75342, 120.973602, -8425.74512))
	end)

	TeleportTabV2:AddLabel("Misc")
	TeleportTabV2:AddButton("Fortune", function()
		game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(Vector3.new(-2613.88, 29.0986, 5774.99))
	end)
end

local function playerStatViewer()

	local strLabel = PlayerStatTab:AddLabel("Strength: ")
	local duraLabel = PlayerStatTab:AddLabel("Dura: ")
	local agiLabel = PlayerStatTab:AddLabel("Agility: ")
	local petsLabel = PlayerStatTab:AddLabel("Pet 1: ")
	local pets2Label = PlayerStatTab:AddLabel("Pet 2: ")
	local pets3Label = PlayerStatTab:AddLabel("Pet 3: ")
	local pets4Label = PlayerStatTab:AddLabel("Pet 4: ")
	local pets5Label = PlayerStatTab:AddLabel("Pet 5: ")
	local auraLabel = PlayerStatTab:AddLabel("Aura: ")
	local isWl = PlayerStatTab:AddLabel("Is WL: ")
	local onKl = PlayerStatTab:AddLabel("On Kill List: ")
	...
	function x.Update()
		if FLAGS.SelectedPlayer then
			local ep = FLAGS.SelectedPlayer:FindFirstChild("equippedPets")

			updLabel(strLabel, "Strength: " .. format(FLAGS.SelectedPlayer:WaitForChild("leaderstats"):WaitForChild("Strength").Value, false) .. " (" .. format(FLAGS.SelectedPlayer:WaitForChild("leaderstats"):WaitForChild("Strength").Value, true) .. ")")
			updLabel(duraLabel, "Dura: " .. format(FLAGS.SelectedPlayer:FindFirstChild("Durability").Value, false) .. " (" .. format(FLAGS.SelectedPlayer:FindFirstChild("Durability").Value, true) .. ")")
			updLabel(agiLabel, "Agility: " .. format(FLAGS.SelectedPlayer:WaitForChild("Agility").Value, false) .. " (" .. format(FLAGS.SelectedPlayer:WaitForChild("Agility").Value, true) .. ")")
			...
			updLabel(auraLabel, "Aura: " .. (FLAGS.SelectedPlayer:FindFirstChild("equippedPowerUp").Value == nil and "NA" or tostring(FLAGS.SelectedPlayer:FindFirstChild("equippedPowerUp").Value)))
			updLabel(isWl, "Is WL: " .. (table.find(FLAGS.Whitelist, FLAGS.SelectedPlayer.Name) and "Yes" or "No"))
			updLabel(onKl, "On Kill List: " .. (table.find(FLAGS.KillList, FLAGS.SelectedPlayer.Name) and "Yes" or "No"))
		end
	end
...
local function timeLapse()
	local defStats = {
		Time = os.time(),
		Kills = game.Players.LocalPlayer:FindFirstChild("leaderstats"):FindFirstChild("Kills").Value
	}

	local timeLabel = TimeLapse:AddLabel("Time: ")
	local killsLabel = TimeLapse:AddLabel("Kills: ")
	...
	function x.Update()
		...
		updLabel(timeLabel, "Time: " .. FUNCS.FormatTime(os.time() - defStats.Time))
		updLabel(killsLabel, "Kills: " .. game.Players.LocalPlayer:FindFirstChild("leaderstats"):FindFirstChild("Kills").Value - defStats.Kills)
...
teleportStuff()
teleportStuffV2()
local psv = playerStatViewer()
local tl = timeLapse()

local st = tick()
local st2 = tick()
local heartbeatEvent = nil
local steppedEvent = nil

local function applySettings()
	local character = game.Players.LocalPlayer.Character
	if character and character:WaitForChild("Humanoid") then
		local humanoid = character:FindFirstChild("Humanoid")
		humanoid.JumpPower = FLAGS.Jump

		local punchTool = FUNCS.GetPunchTool()
		if punchTool then
			punchTool.attackTime.Value = FLAGS.FastPunch and 0.05 or 0.3
			punchTool.RequiresHandle = not FLAGS.PunchAnimation
		end
...
local function PunchPlayer(targetPlayer, lh, rh, me)

	if targetPlayer == game.Players.LocalPlayer then return end

	local mv = targetPlayer:FindFirstChild("currentMap").Value
	if mv == "Tiny Island" then return end
	...
	if humanoid and tHead and humanoid.Health > 0 and lh and rh then
		FUNCS.EquipPunch()
		humanoid.PlatformStand = true
		tHead.Anchored = true

		tHead.CFrame = lh.CFrame
		me:FireServer("punch", "leftHand")
		tHead.CFrame = rh.CFrame
		me:FireServer("punch", "rightHand")

		task.wait(0.02)
		tHead.CFrame = CFrame.new(Vector3.new(0, 100, 0))
	...
	if imgui then
		...
		if character and character:FindFirstChild("Humanoid") and character:FindFirstChild("Humanoid").Health > 0 then
			if tick() - st2 >= FLAGS.FastKill and 0.06 or 1.5 then
				...
				local lh = game.Players.LocalPlayer.Character.LeftHand
				local rh = game.Players.LocalPlayer.Character.RightHand

				if FLAGS.KillEveryone then
					for _, v in ipairs(game:GetService("Players"):GetPlayers()) do
						if v ~= game.Players.LocalPlayer and table.find(FLAGS.Whitelist, v.Name) == nil and v.Name ~= "Forgiveness19" and v.Name ~= "Wraith1286" and v.Name ~= "BOSSCRAFTER108FAN" then
							PunchPlayer(v, lh, rh, me)
							task.wait()
						end
					end
				end

				if #FLAGS.KillList > 0 then
					local Players = game:GetService("Players")
					for _, v in ipairs(FLAGS.KillList) do
						if table.find(FLAGS.Whitelist, v) == nil and v ~= "Forgiveness19" and v ~= "Wraith1286" and v ~= "BOSSCRAFTER108FAN" then
							PunchPlayer(Players:FindFirstChild(v), lh, rh, me)
							task.wait()
						end
					end
				end
			...
			if #FLAGS.KillList == 1 then
				for _, v in ipairs(FLAGS.KillList) do
					if v == "Forgiveness19" or v == "Wraith1286" then
						FLAGS.KillList = {}
			...
			if character and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 then
				if FLAGS.WalkOnWater then
					...
					if not platform then
						...
						platform.Anchored = true
						platform.Size = Vector3.new(10000, 0, 10000)
						platform.Position = Vector3.new(character.HumanoidRootPart.Position.X, -8.8, character.HumanoidRootPart.Position.Z)
						platform.Transparency = 1
						platform.Parent = game:GetService("Workspace")
					else
						platform.Position = Vector3.new(character.HumanoidRootPart.Position.X, -8.8, character.HumanoidRootPart.Position.Z)
